...
import { X, Upload, Trash2, Link, PlusIcon } from "lucide-react";
...
export function AddProjectDialogComponent({ route }: Props) {
    ...
    startTransition(async () => {
      try {
        ...
        if (data.imageSource === "upload" && data.imageFile) {
          ...
          formData.append("routeId", route?.routeId.toString() || "");
          formData.append("userId", route?.userId.toString() || "");
        ...
        const result = await addProject(projectData, route?.routeName || "");
        ...
        toast({
          title: "Error",
          description: `Failed to add project. ${error}`,
          variant: "destructive",
        });
      }
    });
