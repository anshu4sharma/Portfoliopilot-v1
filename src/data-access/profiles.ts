...
export async function getAllUserNamesAndEmails() {
  const result = await db
    .select({
      email: users.email,
      displayName: profiles.displayName,
    ...
    .from(users)
    .leftJoin(profiles, eq(users.id, profiles.userId))
    .all(); // Use `.all()` to fetch all matching rows

  return result;
}

export type UserEmailAndName = Awaited<ReturnType<typeof getUserEmailAndName>>;
